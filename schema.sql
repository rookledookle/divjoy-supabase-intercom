...
create table public.calendars (
  ...
  -- Validate data
  constraint name check (char_length("name") >= 1)
);

-- Create security policies
alter table public.calendars enable row level security;
create policy "Can read calendars they own" on public.calendars for select using ( auth.uid() = "owner" );
create policy "Can insert calendars they own" on public.calendars for insert with check ( auth.uid() = "owner" );